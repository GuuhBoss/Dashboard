...
class Bd {
	...
	public function getTotalReclamacao() {
		...
		return $stmt->fetch(PDO::FETCH_OBJ)->num_reclamacao;
	}

	public function getTotalElogio() {
		$query = '
		select
			count(*) as num_elogios
		from
			tb_contatos
		where
			tipo_contato = 3
		...
		return $stmt->fetch(PDO::FETCH_OBJ)->num_elogios;
	}

		public function getTotalSugestao() {
		$query = '
		select
			count(*) as num_sugestao
		from
			tb_contatos
		where
			tipo_contato = 2
		...
		return $stmt->fetch(PDO::FETCH_OBJ)->num_sugestao;
	}
}
...
$dashboard->__set('total_elogios', $bd->getTotalElogio());
